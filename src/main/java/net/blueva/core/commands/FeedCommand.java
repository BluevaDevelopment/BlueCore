...
public class FeedCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
        if (!(sender instanceof Player) && args.length != 1) {
            sender.sendMessage(MessagesUtil.format(null, ConfigManager.language.getString("messages.other.use_feed_command")));
            return true;
        }
        ...
        target.sendMessage(MessagesUtil.format(target, ConfigManager.language.getString("messages.success.satisfied_appetite")));
        if (args.length == 1) {
            sender.sendMessage(MessagesUtil.format(target, Objects.requireNonNull(ConfigManager.language.getString("messages.success.satisfied_appetite_others")).replace("%player%", target.getName())));
        }
