...
public class EnderChestCommand implements CommandInterface {
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) throws IOException {
        if (!(sender instanceof Player) && args.length != 1) {
            MessagesUtils.sendToSender(null, ConfigManager.language.getString("messages.other.use_enderchest_command"));
            return;
        }
        ...
        MessagesUtils.sendToSender(target, ConfigManager.language.getString("messages.success.enderchest_open"));
        if (args.length == 1) {
            MessagesUtils.sendToSender(target, Objects.requireNonNull(ConfigManager.language.getString("messages.success.enderchest_open_others")).replace("%player%", target.getName()));
        }
