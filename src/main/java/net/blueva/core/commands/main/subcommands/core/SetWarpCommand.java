...
public class SetWarpCommand implements CommandInterface {
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) throws IOException {
        ...
        if(ConfigManager.Modules.warps.getBoolean("warps.enabled")) {
            if(args.length < 1) {
                MessagesUtils.sendToPlayer(player, ConfigManager.language.getString("messages.other.use_setwarp_command"));
                return;
            }

            try {
                if(WarpModule.setWarp(args[0], player)) {
                    MessagesUtils.sendToPlayer(player, Objects.requireNonNull(ConfigManager.language.getString("messages.success.warp_set")).replace("%warp%", args[0]));
                }
            } catch (IOException e) {
